...
export function buildPromptContext(resolution, options) {
    const lines = [
...
export function looksComplexPrompt(prompt, config) {
...
export function isUltraworkPrompt(prompt, config) {
    const normalized = prompt.trim().toLowerCase();
    return config.ultraworkAliases.some((alias) => alias.toLowerCase() === normalized);
}
export function expandUltraworkPrompt() {
    return [
