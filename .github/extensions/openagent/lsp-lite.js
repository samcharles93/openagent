...
function resolveProjectFiles(cwd, targetFile) {
    ...
    if (configPath) {
        ...
        const parsed = ts.parseJsonConfigFileContent(readResult.config, ts.sys, path.dirname(configPath), undefined, configPath);
...
export function getOpenAgentLspDiagnostics(args) {
    ...
    return {
        ...
        diagnostics: diagnostics.slice(0, maxResults).map((diagnostic) => ({
            ...
            start: typeof diagnostic.start === "number"
...
export function getOpenAgentLspReferences(args) {
    ...
    const filtered = includeDeclaration
        ? references
        : references.filter((reference) => !definitionKeys.has(`${reference.fileName}:${reference.textSpan.start}:${reference.textSpan.length}`));
    const maxResults = Math.max(1, args.maxResults ?? 100);
    return {
        filePath: context.absoluteFilePath,
        configPath: context.configPath,
