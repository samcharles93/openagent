...
function getSessionHistoryPathFromWorkspace(workspacePath, config) {
    return path.join(workspacePath, "files", ...config.workspace.notesDirectory.split("/"), "sessions", "history.json");
}
function sanitizeEntry(value) {
    ...
    return {
        ...
        phasesVisited: Array.isArray(value.phasesVisited)
            ? value.phasesVisited.filter((v) => typeof v === "string")
            : [],
        keyFiles: Array.isArray(value.keyFiles)
            ? value.keyFiles.filter((v) => typeof v === "string")
            : [],
    };
}
...
export async function readSessionHistory(session, config) {
    const historyPath = getSessionHistoryPath(session, config);
    return readHistoryFromPath(historyPath);
}
export async function appendSessionHistory(session, config, entry) {
    const historyPath = getSessionHistoryPath(session, config);
...
export async function searchSessionHistory(session, config, query) {
...
export function formatSessionHistoryEntry(entry) {
    const lines = [
...
export async function recordSessionEnd(workspacePath, config, entry) {
