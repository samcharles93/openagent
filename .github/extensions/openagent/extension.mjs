...
import { formatOpenAgentCompactionStatus, noteOpenAgentCompactionComplete, noteOpenAgentCompactionStart, OPENAGENT_BUFFER_EXHAUSTION_THRESHOLD, OPENAGENT_PREEMPTIVE_COMPACTION_THRESHOLD, recordOpenAgentUsage, } from "./compaction.js";
import { loadOpenAgentConfig } from "./config.js";
import { createHooks } from "./hooks.js";
import { buildOpenAgentLoopPrompt, clearOpenAgentLoopState, OPENAGENT_LOOP_DONE_SENTINEL, readOpenAgentLoopState, writeOpenAgentLoopState, } from "./loop-state.js";
import { createPermissionHandler } from "./permissions.js";
import { buildSystemPrompt } from "./prompt.js";
import { recordLoopCancel, recordLoopComplete, recordLoopIteration, } from "./telemetry.js";
...
session.on("session.idle", async (event) => {
    ...
    const lastContent = lastAssistantMessage && lastAssistantMessage.type === "assistant.message"
...
await session.log(initialResolution.sources.length > 0
    ? `OpenAgent harness loaded with config from ${initialResolution.sources.join(", ")}.`
    : "OpenAgent harness loaded with built-in defaults.", { ephemeral: true });
