import { cancelBackgroundTask, formatBackgroundTasksSummary, getBackgroundTaskStats, listBackgroundTasks, registerBackgroundTask, updateBackgroundTask, } from "./background-tasks.js";
import { bootstrapOpenAgentTask, formatOpenAgentBootstrapResult, isOpenAgentBootstrapPhase, listOpenAgentBootstrapPhases, } from "./bootstrap.js";
import { formatBootstrapHistorySummary, readBootstrapHistory, } from "./bootstrap-history.js";
import { applyCategoryOverrides, formatCategorySummary, getCategoryByName, inferCategoryFromObjective, listCategoryNames, } from "./categories.js";
import { formatOpenAgentCompactionStatus, } from "./compaction.js";
import { formatConfigSummary, formatModelTargets, isOpenAgentAgentName, loadOpenAgentConfig, OPENAGENT_AGENT_NAMES, } from "./config.js";
import { runOpenAgentDoctor } from "./doctor.js";
import { runOpenAgentAstReplace, runOpenAgentAstSearch } from "./ast-grep.js";
import { getOpenAgentLspDefinitions, getOpenAgentLspDiagnostics, getOpenAgentLspReferences, runOpenAgentLspRename, } from "./lsp-lite.js";
import { runOpenAgentLookAt } from "./look-at.js";
import { listOpenAgentMemoryTopics, readOpenAgentMemory, writeOpenAgentMemory, } from "./memory.js";
import { formatFallbackStatus } from "./model-fallback.js";
import { updateSessionPlan } from "./plan.js";
import { formatOpenAgentPlanReviewResult, startPlanReviewWorkflow, } from "./plan-review.js";
import { formatSessionHistoryEntry, readSessionHistory, searchSessionHistory, } from "./session-history.js";
import { applyOpenAgentSafeEdit } from "./safe-edit.js";
import { createTaskTools } from "./task-tools.js";
import { formatOpenAgentTelemetry, } from "./telemetry.js";
import { formatOpenAgentRoutingStatus, isOpenAgentPhase, listOpenAgentPhases, routeOpenAgentPhase, } from "./routing.js";
import { formatOpenAgentWorkspaceRequirement, isOpenAgentWorkspaceAvailable, writeOpenAgentWorkspaceNote, } from "./workspace.js";
...
function parseBootstrapTaskArgs(args) {
    ...
    const mode = args.mode === "interactive" || args.mode === "plan" || args.mode === "autopilot"
        ? args.mode
        : "default";
    const rawPhase = args.phase;
    if (typeof rawPhase !== "undefined" &&
        rawPhase !== "auto" &&
    ...
    return {
        ...
        phase: rawPhase === "auto" || (typeof rawPhase === "string" && isOpenAgentBootstrapPhase(rawPhase))
            ? rawPhase
            : "auto",
...
function parseAstSearchArgs(args, toolName) {
    ...
    return {
        ...
        globs: Array.isArray(args.globs)
            ? args.globs.filter((entry) => typeof entry === "string" && entry.length > 0)
            : undefined,
        paths: Array.isArray(args.paths)
            ? args.paths.filter((entry) => typeof entry === "string" && entry.length > 0)
            : undefined,
        json: args.json === true,
    };
}
...
export function createTools(args) {
    ...
    const bootstrapTaskTool = {
        ...
        handler: async (args) => {
            ...
            return createSuccessResult(formatOpenAgentBootstrapResult(result), "OpenAgent bootstrapped the task into a plan and routed phase.");
        },
    };
    const planNoteTool = {
    ...
    const workspaceNoteTool = {
        ...
        handler: async (args) => {
            ...
            const session = getSession();
            if (!isOpenAgentWorkspaceAvailable(session)) {
                return createFailureResult("OpenAgent could not persist the workspace note because the session workspace is disabled.", "Session workspace is unavailable.");
            }
            const resolution = loadOpenAgentConfig(resolveCwd(initialCwd));
        ...
        parameters: {
            ...
            properties: {
                ...
                agent: {
                    ...
                    description: "Optional agent override inside the target phase (for example openagent-critic or openagent-oracle).",
                },
                objective: {
                    type: "string",
    ...
    const planReviewTool = {
        ...
        handler: async (args) => {
            ...
            return createSuccessResult(formatOpenAgentPlanReviewResult(result), "OpenAgent started the plan-review workflow.");
        },
    };
    const doctorTool = {
        ...
        handler: async (args) => {
            ...
            const writeReport = isRecord(args) && typeof args.writeReport === "boolean" ? args.writeReport : true;
    ...
    const memoryReadTool = {
        ...
        handler: async (args) => {
            ...
            if (result.content === null) {
                return createFailureResult(`No repository memory note found for topic "${parsedArgs.topic}".`, "Memory note not found.");
            }
            return createSuccessResult([
    ...
    const memoryListTool = {
        ...
        handler: async () => {
            ...
            if (result.topics.length === 0) {
                return createSuccessResult(`No repository memory topics stored for repo key "${result.repoKey}".`);
            }
            return createSuccessResult([
    ...
    const lspFindReferencesTool = {
        ...
        handler: async (args) => {
            try {
                ...
                const includeDeclaration = isRecord(args) && typeof args.includeDeclaration === "boolean"
    ...
    const astSearchTool = {
        ...
        handler: async (args) => {
            try {
                ...
                return createSuccessResult([`Command: ${result.command}`, "", output].join("\n"), "OpenAgent ran ast-grep search.");
            }
            catch (err) {
                const message = err instanceof Error ? err.message : String(err);
                return createFailureResult(message, message);
    ...
    const backgroundRegisterTool = {
        ...
        handler: async (args) => {
            if (!isRecord(args) || typeof args.id !== "string" || args.id.length === 0) {
                return createFailureResult("openagent_background_register requires a non-empty string id.", "Missing id.");
            }
            if (typeof args.description !== "string" || args.description.length === 0) {
                return createFailureResult("openagent_background_register requires a non-empty string description.", "Missing description.");
            }
            const owner = typeof args.owner === "string" && args.owner.length > 0
                ? args.owner
                : "openagent";
            try {
                ...
                return createSuccessResult(`Registered background task "${task.id}" (owner: ${task.owner}, status: running).`, `OpenAgent registered background task ${task.id}.`);
            }
            catch (err) {
                const message = err instanceof Error ? err.message : String(err);
                return createFailureResult(message, message);
    ...
    const backgroundUpdateTool = {
        ...
        handler: async (args) => {
            if (!isRecord(args) || typeof args.id !== "string" || args.id.length === 0) {
                return createFailureResult("openagent_background_update requires a non-empty string id.", "Missing id.");
            }
            const update = {};
            ...
            try {
                ...
                return createSuccessResult(`Updated background task "${task.id}" (status: ${task.status}).`, `OpenAgent updated background task ${task.id}.`);
            }
            catch (err) {
                const message = err instanceof Error ? err.message : String(err);
                return createFailureResult(message, message);
    ...
    const backgroundListTool = {
        ...
        handler: async (args) => {
            ...
            return createSuccessResult(summary, `OpenAgent listed ${tasks.length} background task(s).`);
        },
    };
    const backgroundCancelTool = {
        ...
        handler: async (args) => {
            if (!isRecord(args) || typeof args.id !== "string" || args.id.length === 0) {
                return createFailureResult("openagent_background_cancel requires a non-empty string id.", "Missing id.");
            }
            try {
                const task = cancelBackgroundTask(args.id);
                return createSuccessResult(`Cancelled background task "${task.id}".`, `OpenAgent cancelled background task ${task.id}.`);
            }
            catch (err) {
                const message = err instanceof Error ? err.message : String(err);
                return createFailureResult(message, message);
    ...
    const sessionListTool = {
        ...
        handler: async (args) => {
            const session = getSession();
            if (!isOpenAgentWorkspaceAvailable(session)) {
                return createFailureResult(formatOpenAgentWorkspaceRequirement("Session history listing"), "Session workspace is unavailable.");
            }
            const resolution = loadOpenAgentConfig(resolveCwd(initialCwd));
            const history = await readSessionHistory(session, resolution.config);
            const count = isRecord(args) && typeof args.count === "number" && args.count > 0
            ...
            const formatted = recent
                ...
                .join("\n\n");
            return createSuccessResult(`Found ${recent.length} session history entries (of ${history.entries.length} total).\n\n${formatted}`);
        },
    };
    const sessionSearchTool = {
        ...
        handler: async (args) => {
            if (!isRecord(args) || typeof args.query !== "string" || args.query.trim().length === 0) {
                return createFailureResult("openagent_session_search requires a non-empty string query field.", "Missing query.");
            }
            const session = getSession();
            if (!isOpenAgentWorkspaceAvailable(session)) {
                return createFailureResult(formatOpenAgentWorkspaceRequirement("Session history search"), "Session workspace is unavailable.");
            }
            const resolution = loadOpenAgentConfig(resolveCwd(initialCwd));
            ...
            const formatted = matches
                ...
                .join("\n\n");
            return createSuccessResult(`Found ${matches.length} matching session history entries.\n\n${formatted}`);
        },
    };
    const sessionGetTool = {
        ...
        handler: async (args) => {
            if (!isRecord(args) || typeof args.sessionId !== "string" || args.sessionId.trim().length === 0) {
                return createFailureResult("openagent_session_get requires a non-empty string sessionId field.", "Missing sessionId.");
            }
            const session = getSession();
            if (!isOpenAgentWorkspaceAvailable(session)) {
                return createFailureResult(formatOpenAgentWorkspaceRequirement("Session history lookup"), "Session workspace is unavailable.");
            }
            const resolution = loadOpenAgentConfig(resolveCwd(initialCwd));
            const history = await readSessionHistory(session, resolution.config);
            const entry = history.entries.find((e) => e.sessionId === args.sessionId);
            if (!entry) {
                return createFailureResult(`No session history entry found with ID "${args.sessionId}".`, "Session not found.");
            }
    ...
    const categoriesListTool = {
        ...
        handler: async () => {
            const resolution = loadOpenAgentConfig(resolveCwd(initialCwd));
            return createSuccessResult(formatCategorySummary(applyCategoryOverrides(resolution.config.categories)));
        },
    };
    const allTools = [
        runtimeStatusTool,
