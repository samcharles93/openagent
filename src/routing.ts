...
export type OpenAgentPhase = (typeof OPENAGENT_PHASES)[number];
export type OpenAgentMode = "interactive" | "plan" | "autopilot";

type PhaseDefinition = {
  agent: OpenAgentAgentName;
  agents: readonly OpenAgentAgentName[];
  mode: OpenAgentMode;
  description: string;
};

type OpenAgentRouteTransition = {
  timestamp: string;
  ...
  toMode: OpenAgentMode;
  objective: string;
  handoffPath: string;
  requestedBy: string;
};

export type OpenAgentRouteState = {
  currentPhase: OpenAgentPhase;
  currentAgent: OpenAgentAgentName;
  currentMode: OpenAgentMode;
  latestHandoffPath: string | null;
  updatedAt: string;
  transitions: OpenAgentRouteTransition[];
  bootstrapContext: OpenAgentBootstrapContext | null;
};

export type OpenAgentBootstrapContext = {
  lastBootstrapPhase: string;
  lastBootstrapConfidence: number;
  lastBootstrapReason: string;
  totalBootstraps: number;
};

export type OpenAgentRouteRequest = {
  ...
  requestedBy?: string;
  bootstrapContext?: OpenAgentBootstrapContext;
};

export type OpenAgentRouteResult = {
  phase: OpenAgentPhase;
  agent: OpenAgentAgentName;
  mode: OpenAgentMode;
...
function isOpenAgentMode(value: unknown): value is OpenAgentMode {
  return value === "interactive" || value === "plan" || value === "autopilot";
}

export function isOpenAgentPhase(value: string): value is OpenAgentPhase {
  return OPENAGENT_PHASES.includes(value as OpenAgentPhase);
}

export function listOpenAgentPhases(): string {
  return OPENAGENT_PHASES.join(", ");
}

function inferAgentName(rawAgentName: string | null | undefined): OpenAgentAgentName {
...
export function inferOpenAgentPhase(rawAgentName: string | null | undefined): OpenAgentPhase {
...
function resolveTargetAgent(
  phase: OpenAgentPhase,
  requestedAgent: OpenAgentAgentName | undefined,
): OpenAgentAgentName {
  ...
  if (!definition.agents.includes(requestedAgent)) {
    throw new Error(
      `Agent "${requestedAgent}" is not available for phase "${phase}". Allowed agents: ${definition.agents.join(", ")}.`,
    );
  }
...
function sanitizeTransitions(value: unknown): OpenAgentRouteTransition[] {
  if (!Array.isArray(value)) {
    return [];
  }

  return value.filter((entry): entry is OpenAgentRouteTransition => {
    if (!isRecord(entry)) {
      return false;
    }
...
function sanitizeBootstrapContext(value: unknown): OpenAgentBootstrapContext | null {
  if (!isRecord(value)) {
    return null;
  }

  if (
    typeof value.lastBootstrapPhase !== "string" ||
...
function sanitizeRouteState(value: unknown): OpenAgentRouteState | null {
  if (!isRecord(value)) {
    return null;
  }
...
}): Promise<OpenAgentRouteState | null> {
...
export async function routeOpenAgentPhase(args: {
  ...
  const targetMode =
    request.mode && request.mode !== "default"
      ? request.mode
  ...
  const nextState: OpenAgentRouteState = {
...
export async function formatOpenAgentRoutingStatus(args: {
  ...
  if (state?.bootstrapContext) {
    ...
    lines.push(
      `bootstrap-originated phase: ${ctx.lastBootstrapPhase}`,
      `bootstrap confidence: ${ctx.lastBootstrapConfidence.toFixed(2)}`,
      `bootstrap reason: ${ctx.lastBootstrapReason}`,
      `total bootstraps in route: ${ctx.totalBootstraps}`,
    );
  }
