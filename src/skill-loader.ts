...
function scanSkillDirectory(dir: string): string[] {
  try {
    if (!existsSync(dir)) {
      return [];
    }
...
export function discoverSkillFiles(cwd: string): string[] {
...
export async function loadSkills(cwd: string): Promise<OpenAgentSkill[]> {
  const files = discoverSkillFiles(cwd);
  const skills: OpenAgentSkill[] = [];
...
export function formatSkillsForPrompt(skills: OpenAgentSkill[]): string {
  if (skills.length === 0) {
    return "";
  }
...
export function matchSkillByTrigger(
  skills: OpenAgentSkill[],
  userPrompt: string,
): OpenAgentSkill[] {
