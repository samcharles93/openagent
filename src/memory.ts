...
function buildRepoKey(cwd: string): string {
...
function getMemoryRoot(cwd: string): { repoKey: string; root: string } {
...
async function collectTopics(root: string, prefix = ""): Promise<string[]> {
  if (!existsSync(root)) {
    return [];
  }
