...
export type OpenAgentCustomCommand = {
  name: string;
  description: string;
  content: string;
  source: string;
...
export function parseCommandFrontmatter(raw: string): CommandFrontmatter | null {
...
function scanCommandDirectory(dir: string): string[] {
  try {
    if (!existsSync(dir)) {
      return [];
    }
...
export function loadCustomCommands(cwd: string): OpenAgentCustomCommand[] {
  const commands: OpenAgentCustomCommand[] = [];
...
export function renderCustomCommandPrompt(
  command: OpenAgentCustomCommand,
  args: string,
): string {
