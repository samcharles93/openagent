...
function tryResolve(value: () => string): string | null {
  try {
...
export function resolveBundledAstGrepBinary(): string | null {
  ...
  const platformPackage = tryResolve(() =>
    require.resolve(`@ast-grep/cli-${parts.join("-")}/package.json`),
  );
  if (!platformPackage) {
    return directBinary;
  }
