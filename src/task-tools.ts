...
const TASK_STATUSES: readonly OpenAgentTaskStatus[] = [
  "pending",
  "in_progress",
  "done",
  "blocked",
...
function isTaskStatus(value: unknown): value is OpenAgentTaskStatus {
  return typeof value === "string" && TASK_STATUSES.includes(value as OpenAgentTaskStatus);
}
...
function formatTask(task: OpenAgentTask): string {
  const lines = [
    `id: ${task.id}`,
    `title: ${task.title}`,
    `status: ${task.status}`,
...
function formatTaskSummary(task: OpenAgentTask): string {
  const blockerNote =
...
function parseCreateTaskArgs(args: unknown): CreateTaskArgs {
  if (
    !isRecord(args) ||
    ...
    throw new Error(
      "openagent_task_create requires non-empty string id, title, and description fields.",
    );
  }

  const blockedBy =
    args.blockedBy !== undefined && isStringArray(args.blockedBy) ? args.blockedBy : [];
  const blocks =
    args.blocks !== undefined && isStringArray(args.blocks) ? args.blocks : [];
  const owner = typeof args.owner === "string" && args.owner.length > 0 ? args.owner : "user";
  const metadata =
    args.metadata !== undefined && isStringRecord(args.metadata) ? args.metadata : {};
...
function parseUpdateTaskArgs(args: unknown): UpdateTaskArgs {
  ...
  const result: UpdateTaskArgs = { id: args.id };

  if (args.status !== undefined) {
    if (!isTaskStatus(args.status)) {
      throw new Error(
        `openagent_task_update status must be one of: ${TASK_STATUSES.join(", ")}.`,
      );
    }
...
function parseListTaskArgs(args: unknown): ListTaskArgs {
  if (!isRecord(args)) {
    return { showReady: false };
  }

  const result: ListTaskArgs = {
...
function parseGetTaskArgs(args: unknown): { id: string } {
  if (!isRecord(args) || typeof args.id !== "string" || args.id.length === 0) {
...
export function createTaskTools(args: {
  ...
  const taskCreateTool: Tool = {
    ...
    description:
      "Create a new task in the OpenAgent task tracker with dependency information and ownership.",
    skipPermission: true,
    parameters: {
      type: "object",
      properties: {
    ...
    handler: async (args) => {
      ...
      if (!session.workspacePath) {
        return createFailureResult(
          "Cannot create task because the session workspace is disabled.",
      ...
      if (existing) {
        return createFailureResult(
          `Task "${parsed.id}" already exists. Use openagent_task_update to modify it.`,
          "Task ID already exists.",
        );
      }

      const now = new Date().toISOString();
      const task: OpenAgentTask = {
        id: parsed.id,
        title: parsed.title,
      ...
      return createSuccessResult(
        `Created task "${task.id}" (${task.title}) with status pending.`,
        `OpenAgent created task ${task.id}.`,
      );
    },
  };
  ...
  const taskListTool: Tool = {
    ...
    description:
      "List all tasks in the OpenAgent task tracker with optional status filter or ready-only mode.",
    skipPermission: true,
    parameters: {
      type: "object",
      properties: {
        ...
        showReady: {
          type: "boolean",
          description:
            "If true, only return pending tasks whose blockers are all done.",
        },
      },
    },
    handler: async (args) => {
      ...
      if (!session.workspacePath) {
        return createFailureResult(
          "Cannot list tasks because the session workspace is disabled.",
          "Session workspace is unavailable.",
        );
      }
      ...
      if (tasks.length === 0) {
        const qualifier = parsed.showReady
          ...
          : parsed.status
            ...
            : "";
        return createSuccessResult(
          `No tasks found${qualifier ? ` ${qualifier}` : ""}.`,
        );
      }
  ...
  const taskGetTool: Tool = {
    ...
    handler: async (args) => {
      ...
      if (!session.workspacePath) {
        return createFailureResult(
          "Cannot get task because the session workspace is disabled.",
          "Session workspace is unavailable.",
        );
      }
      ...
      return createSuccessResult(
        formatTask(task),
        `OpenAgent retrieved task ${task.id}.`,
      );
    },
  };
  ...
  const taskUpdateTool: Tool = {
    ...
    description:
      "Update fields on an existing task in the OpenAgent task tracker. Only provided fields are changed.",
    skipPermission: true,
    parameters: {
      type: "object",
      properties: {
    ...
    handler: async (args) => {
      ...
      if (!session.workspacePath) {
        return createFailureResult(
          "Cannot update task because the session workspace is disabled.",
          "Session workspace is unavailable.",
        );
      }
      ...
      return createSuccessResult(
        `Updated task "${parsed.id}": ${changes.join(", ")}.`,
        `OpenAgent updated task ${parsed.id}.`,
      );
    },
  };
